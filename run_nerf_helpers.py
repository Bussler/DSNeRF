...
mse2psnr = lambda x : -10. * torch.log(x) / torch.log(torch.Tensor([10.]).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))) # M: explicitly put to cuda here to avoid bugs TODO clean up
...
def get_rays(H, W, focal, c2w, device):
    ...
    rays_d = torch.sum(dirs[..., np.newaxis, :].to(device) * c2w[:3,:3].to(device), -1)  # dot product, equals to: [c2w.dot(dir) for dir in dirs] # M: explicitly move to cuda
    # Translate camera frame's origin to the world frame. It is the origin of all rays.
    rays_o = c2w[:3,-1].expand(rays_d.shape).to(device) # M: explicitly move to cuda
    return rays_o, rays_d


def get_rays_np(H, W, focal, c2w, device):
    ...
    # Rotate ray directions from camera frame to the world frame
    rays_d = np.sum(dirs[..., np.newaxis, :] * c2w[:3,:3], -1)  # dot product, equals to: [c2w.dot(dir) for dir in dirs] # M: explicitly move to cuda
    # Translate camera frame's origin to the world frame. It is the origin of all rays.
    rays_o = np.broadcast_to(c2w[:3,-1], np.shape(rays_d))  # H x W x 3
    return rays_o, rays_d


def get_rays_by_coord_np(H, W, focal, c2w, coords, device):
    i, j = (coords[:,0]-W*0.5)/focal, -(coords[:,1]-H*0.5)/focal
    dirs = np.stack([i,j,-np.ones_like(i)],-1)
    rays_d = np.sum(dirs[..., np.newaxis, :] * c2w[:3,:3], -1) # M: explicitly move to cuda
...
def sample_pdf(bins, weights, N_samples, device, det=False, pytest=False):
    ...
    # Invert CDF
    u = u.contiguous().to(device) # M: explicitly put to cuda here to avoid bugs
...
def raw2outputs(raw, z_vals, rays_d, device, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = torch.cat([dists, torch.Tensor([1e10]).expand(dists[...,:1].shape).to(device)], -1)  # [N_rays, N_samples] # M: explicitly put to cuda here to avoid bugs
...
def sample_sigma(rays_o, rays_d, viewdirs, network, z_vals, network_query, device):
